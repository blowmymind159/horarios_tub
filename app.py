...
def detetar_tipo_dia(data_escolhida):
    """Deteta automaticamente o tipo de dia baseado na data"""
...
# ============================================
st.markdown("---")
st.subheader(" Quando pretende viajar?")

modo_tempo = st.radio(
    "Escolha o modo:",
    ["Agora (hora atual)", "Data e hora específicas"],
    horizontal=True
)

if modo_tempo == "Agora (hora atual)":
    agora = datetime.now()
    hora_consulta = agora.strftime("%H:%M")
    data_consulta = agora.date()
    st.info(f" A consultar horários a partir das **{hora_consulta}** de hoje ({agora.strftime('%d/%m/%Y')})")
else:
    col1, col2 = st.columns(2)
    with col1:
        data_consulta = st.date_input("Data:", min_value=date.today())
    with col2:
        hora_input = st.time_input("Hora:", value=time(8, 0))
    
    hora_consulta = hora_input.strftime("%H:%M")
    
    # Detetar tipo de dia automaticamente
    tipo_dia_automatico = detetar_tipo_dia(data_consulta)
    nome_dia = ["Segunda-feira", "Terça-feira", "Quarta-feira", "Quinta-feira", "Sexta-feira", "Sábado", "Domingo"][data_consulta.weekday()]
    
    st.success(f"📅 A consultar horários para **{data_consulta.strftime('%d/%m/%Y')}** ({nome_dia}) a partir das **{hora_consulta}**")
    st.info(f"💡 Tipo de dia detetado automaticamente: **{tipo_dia_automatico}** ({DESCRICOES_TIPOS_DIA.get(tipo_dia_automatico, '')})")

st.markdown("---")

# SELEÇÃO DE LINHA
st.subheader("1️ Escolha a Linha")
linhas = get_linhas()
...
# Se estiver no modo "Data específica", sugerir o tipo detetado automaticamente
if modo_tempo == "Data e hora específicas":
    tipo_dia_sugerido = tipo_dia_automatico
    if tipo_dia_sugerido in tipos_dia:
        st.info(f"✅ O tipo de dia **{tipo_dia_sugerido}** está disponível para esta paragem.")
    else:
        st.warning(f"⚠️ O tipo de dia **{tipo_dia_sugerido}** não está disponível para esta paragem. Escolha um dos disponíveis abaixo.")
...
# Pré-selecionar o tipo correto se estiver no modo data específica
indice_preselecionado = 0
if modo_tempo == "Data e hora específicas":
    for idx, tipo in enumerate(tipos_dia):
        if tipo == tipo_dia_automatico:
            indice_preselecionado = idx
            break

tipo_dia_sel_display = st.selectbox("Que tipo de dia é?", opcoes_tipos, index=indice_preselecionado)
tipo_dia_codigo = tipo_dia_sel_display.split(" - ")[0]

st.markdown("---")

# RESULTADOS
st.subheader(f"🕐 Horários - {paragem_sel}")
...
<div style="padding: 1rem; border-radius: 8px; background-color: #f0f2f6; margin-bottom: 1rem;">
    ...
    <strong>🕒 A partir das:</strong> {hora_consulta}
</div>
""", unsafe_allow_html=True)

resultados = get_horarios(linha_codigo, paragem_sel, direcao_sel, tipo_dia_codigo, hora_consulta)
...
if resultados:
    ...
    st.warning(f"⚠️ Não há horários para esta paragem ({tipo_dia_sel_display}) a partir das {hora_consulta}.")
    st.info("💡 Tente consultar outro tipo de dia, outra paragem ou outra hora.")
